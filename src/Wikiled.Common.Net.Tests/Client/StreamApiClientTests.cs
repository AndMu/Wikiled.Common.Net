...
public class StreamApiClientTests
{
    ...
    public async Task GetRawRequest()
    {
        ...
        ClassicAssert.AreEqual("Test Result", result[0].Name);
    }
    ...
    public void PostRequestWithErrorCode()
    {
        ...
        var argument = new TestData();
        ClassicAssert.ThrowsAsync<HttpRequestException>(async () => await instance.PostRequest<TestData, TestData>("test", argument, CancellationToken.None).ToArray());
    }
    ...
    public void ExceptionResponse()
    {
        ...
        var argument = new TestData();
        ClassicAssert.ThrowsAsync<Exception>(async () => await instance.PostRequest<TestData, TestData>("test", argument, CancellationToken.None).ToArray());
    }

    [Test]
    public void Construct()
    {
        ClassicAssert.Throws<ArgumentNullException>(() => new StreamApiClient(null, baseUri, logger));
        ClassicAssert.Throws<ArgumentNullException>(() => new StreamApiClient(httpClient, null, logger));
        ClassicAssert.Throws<ArgumentNullException>(() => new StreamApiClient(httpClient, baseUri, null));
    }
